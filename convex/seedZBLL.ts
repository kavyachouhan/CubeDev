...
export const seedZBLLAlgorithms = mutation({
  ...
  handler: async (ctx) => {
    ...
    const zbllSetId = await ctx.db.insert("algorithmSets", {
      ...
      category: "Last Layer",
      description:
        "ZBLL (Zborowski-Bruchem Last Layer) is a collection of algorithms used to solve the last layer of the Rubik's Cube in one step, after orienting the last layer (OLL). It consists of 493 algorithms that cover all possible permutations of the last layer pieces while keeping their orientation intact.",
      caseCount: 493,
      difficulty: "advanced",
      puzzleType: "3x3x3",
      order: 4,
