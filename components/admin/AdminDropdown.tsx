    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      {isOpen && (
        <>
          ...
          {/* Dropdown menu */}
          <div
            className={`absolute top-full left-0 mt-1 w-full min-w-[200px] bg-(--surface) border border-(--border) rounded-lg shadow-lg z-50 overflow-hidden ${menuClassName}`}
          >
            {/* Search input */}
            {searchable && (
...
export function AdminSelect<T extends string = string>({
  ...
  return (
    <select
      ...
      disabled={disabled}
      className={`px-3 py-2 bg-(--surface-elevated) border border-(--border) rounded-lg text-sm text-(--text-primary) font-inter focus:outline-none focus:ring-2 focus:ring-(--primary) focus:border-transparent transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
    >
      {placeholder && (
    ...
    <div ref={dropdownRef} className={`relative ${className}`}>
      ...
      {isOpen && (
        <>
          ...
          />
          <div className="absolute top-full left-0 mt-1 min-w-full w-max bg-(--surface) border border-(--border) rounded-lg shadow-lg z-50 max-h-60 overflow-y-auto">
            {options.map((option) => (
              <button
                key={String(option.value)}
                type="button"
                ...
                className={`w-full flex items-center justify-between gap-3 px-3 py-2 text-left text-sm hover:bg-(--surface-elevated) transition-colors ${
                  option.value === value
                    ? "bg-(--primary)/10 text-(--primary)"
                    : "text-(--text-primary)"
                }`}
