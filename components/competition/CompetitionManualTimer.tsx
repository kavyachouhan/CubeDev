...
interface CompetitionManualTimerProps {
  ...
  isDisabled?: boolean;
  onStateChange?: (state: "idle" | "input" | "inspecting") => void;
}

export default function CompetitionManualTimer({
  onSolveComplete,
  inspectionEnabled = true,
  isDisabled = false,
  onStateChange,
}: CompetitionManualTimerProps) {
  ...
  useEffect(() => {
    onStateChange?.(isInspecting ? "inspecting" : timeInput ? "input" : "idle");
  }, [isInspecting, timeInput, onStateChange]);
  ...
  }, [inspectionEnabled, isInspecting, handleStartInspection]);

  // Parse time input (matching timer page's ManualTimerCore)
  ...
  const formatTimeDisplay = (
    timeMs: number,
    displayPenalty: "none" | "+2" | "DNF"
  ): string => {
    if (displayPenalty === "DNF" || timeMs === Infinity || timeMs === 0)
      return "DNF";
    ...
    return displayPenalty === "+2" ? `${formatted}+` : formatted;
  };
  ...
  const handleSubmit = () => {
    try {
      ...
      onSolveComplete(result.time, finalPenalty, null);
      ...
      console.error("Error submitting manual time:", err);
  ...
  return (
    <div
      className={`timer-card ${isDisabled ? "opacity-50 pointer-events-none" : ""}`}
    >
      {/* Inspection Display */}
      {isInspecting && (
        <div className="text-center p-6">
          ...
          <button
            ...
            className="mt-4 px-4 py-2 bg-[var(--error)] hover:opacity-90 text-white rounded-lg font-medium transition-opacity"
          >
            Stop Inspection
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="mt-2 text-xs text-[var(--text-muted)]">
              Formats: 12.34, 1:23.45, 1234, DNF, 12.34+2
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2 sm:py-3 bg-[var(--primary)] hover:opacity-90 text-white rounded-lg font-medium transition-opacity disabled:opacity-50 disabled:cursor-not-allowed"
            >
